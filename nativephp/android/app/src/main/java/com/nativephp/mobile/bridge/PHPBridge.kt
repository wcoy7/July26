...
class PHPBridge(private val context: Context) {
    ...
    /**
     * Run an artisan command in an ephemeral PHP context (safe for WorkManager / background).
     * Boots, runs, and shuts down so background workers do not contend with the UI runtime.
     */
    fun runEphemeralArtisan(command: String): String {
        ensureRuntimeInitialized()
        val bootstrap = persistentBootstrapScript
        val boot = nativeEphemeralBoot(bootstrap)
        if (boot != 0) {
            Log.e(TAG, "Ephemeral boot failed (code=$boot), falling back to classic artisan")
            return runArtisanCommand(command)
        }
        return try {
            nativeEphemeralArtisan(command)
        } finally {
            try {
                nativeEphemeralShutdown()
            } catch (e: Exception) {
                Log.w(TAG, "Ephemeral shutdown warning: ${e.message}")
            }
        }
    }
